...
const w1 = (sketch) => {
  ...
  sketch.setup = () => {
    ...
    sketch.createCanvas(16 * numCols, 16 * numRows).parent("canvasContainer1").elt.getContext("2d").imageSmoothingEnabled = false;
  ...
  sketch.preload = () => {
    tilesetImage = sketch.loadImage("./assets/tileset.png");
  };
  ...
  function generateGrid(numCols, numRows) {
    ...
    for (let i = 0; i < numRows; i++) {
      ...
      for (let j = 0; j < numCols; j++) {
        let outerValue = sketch.noise(i / 50, j / 50);
        let innerValue = sketch.noise(i / 20, j / 20);
    ...
    let centerRow = sketch.floor(numRows / 2);
    let centerCol = sketch.floor(numCols / 2);
      ...
      for (let i = 1; i < numRows - 1 && !housePlaced; i++) {
        for (let j = 1; j < numCols - 1 && !housePlaced; j++) {
          if (canPlaceHouse(grid, i, j)) {
            ...
            houseColor = sketch.floor(sketch.random() * 4);
            housePlaced = true;
          }
        }
      }
    ...
    for (let i = 1; i < numRows - 1; i++) {
      for (let j = 1; j < numCols - 1; j++) {
        ...
        let lilyPadNoise = sketch.noise(i * lilyPadNoiseScale, j * lilyPadNoiseScale);
  ...
  function drawGrid(grid) {
    ...
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[i].length; j++) {
        if (season == 3) {
          ...
          placeTile(i, j, (4 * sketch.pow(sketch.noise(t / 10, i, j / 4 + t), 2)) | 0, 14); // water
        }


        if (gridCheck(grid, i, j, ":")) { // dirt
          if (season == 3) {
            placeTile(i, j, (4 * sketch.pow(sketch.random(), g)) | 0, 12);
          }
          else {
            placeTile(i, j, (4 * sketch.pow(sketch.random(), g)) | 0, 3);
          }
        } else {
        ...
        if (gridCheck(grid, i, j, ".")) { // grass
          if (season == 1) {
            placeTile(i, j, (4 * sketch.pow(sketch.random(), g)) | 0, 0);
          }
          else if (season == 2) {
            placeTile(i, j, (4 * sketch.pow(sketch.random(), g)) | 0, 6);
          }
          else if (season == 3) {
            placeTile(i, j, (4 * sketch.pow(sketch.random(), g)) | 0, 12);
          }
          else {
            placeTile(i, j, (4 * sketch.pow(sketch.random(), g)) | 0, 1);
          }
        } 
    ...
    currentSecond = sketch.floor(sketch.millis() / 5000)
...
const w2 = (sketch) => {
  ...
  sketch.setup = () => {
    ...
    sketch.createCanvas(16 * numCols, 16 * numRows).parent("canvasContainer2").elt.getContext("2d").imageSmoothingEnabled = false;
  ...
  function generateGrid(numCols, numRows) {
    ...
    for (let roomCount = 0; roomCount < numRooms; roomCount++) {
      ...
      while (!roomPlaced && attempts < maxAttempts) {
        ...
        const roomWidth = sketch.floor(sketch.random(5, 7)); // Random room width between 5 and 10
        const roomHeight = sketch.floor(sketch.random(5, 7)); // Random room height between 5 and 10
        const roomX = sketch.floor(sketch.random(numCols - roomWidth)); // Random room X coordinate
        const roomY = sketch.floor(sketch.random(numRows - roomHeight)); // Random room Y coordinate
        ...
        if (!roomTouchesOtherRooms(grid, roomX, roomY, roomWidth, roomHeight) && !roomTouchesBorders(grid, roomX, roomY, roomWidth, roomHeight)) {
          ...
          const doorPositions = [
            { x: roomX + sketch.floor(roomWidth / 2), y: roomY }, // Top wall
            { x: roomX + roomWidth - 1, y: roomY + sketch.floor(roomHeight / 2) }, // Right wall
            { x: roomX + sketch.floor(roomWidth / 2), y: roomY + roomHeight - 1 }, // Bottom wall
            { x: roomX, y: roomY + sketch.floor(roomHeight / 2) } // Left wall
          ];
          ...
          let replaced = false;
          while (!replaced) {
            const replaceX = roomX + sketch.floor(sketch.random(roomWidth));
            const replaceY = roomY + sketch.floor(sketch.random(roomHeight));
            ...
            if (grid[replaceY][replaceX] === '+') {
              ...
              if (!touchingDoor) {
                const replacementChar = sketch.random() < 0.5 ? 'c' : 'g';
