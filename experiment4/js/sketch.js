...
const w1 = (sketch) => {
    ...
    sketch.draw = () => {
        ...
        if (sketch.keyIsDown(65)) {
            handleKeyPressWithCooldown('ArrowLeft', 300, () => {
                p3_tileClicked(duck.i + 1, duck.j - 1);
            });
        }
      
        else if (sketch.keyIsDown(68)) {
            handleKeyPressWithCooldown('ArrowRight', 300, () => {
                p3_tileClicked(duck.i - 1, duck.j + 1);
            });
        }
      
        else if (sketch.keyIsDown(87)) {
            handleKeyPressWithCooldown('ArrowUp', 300, () => {
                p3_tileClicked(duck.i - 1, duck.j - 1);
            });
        }
      
        else if (sketch.keyIsDown(83)) {
            handleKeyPressWithCooldown('ArrowDown', 300, () => {
                p3_tileClicked(duck.i + 1, duck.j + 1);
            });
        }
...
function draw() {
  ...
  if (version === 3) {
    ...
    textAlign(LEFT, TOP); // Align text to center horizontally and top vertically
    text("Score: " + score, 20, 20); // Display the score text at the top
...
function p3_preload() {
  tilesetImage = loadImage(
    "https://cdn.glitch.global/1555c7ac-26f7-4bac-9385-da3362a4b120/exp4_tilesheet.png?v=1714178331256"
  );
...
function p3_tileClicked(i, j) {
  if (version === 1) {
    ...
    // Check if the clicked tile is within 1 tile distance from the duck's current position
    if ((!isGarbageTile) && Math.abs(di) <= 1 && Math.abs(dj) <= 1 && (di !== 0 || dj !== 0)) {
      // Update the duck's position to the clicked tile
      duck.i = i;
      duck.j = j;
...
function drawDuck(x, y, direction) {
  ...
  translate(x, y); // Move the entire duck to the specified position
  ...
  image(tilesetImage, -16, -24, 32, 32, 16 * col, 16 * row, 16, 16)
...
function drawPlanet(planetIndex, colorIndex, ringIndex, scale) {
  ...
  // Draw the base planet
  image(tilesetImage, -size / 2, -size / 2, size, size, 32 * planet_col, 32 * planet_row, 32, 32);
  // Draw the color
  image(tilesetImage, -size / 2, -size / 2, size, size, 32 * color_col, 32 * color_row, 32, 32);
  // Draw the ring
  image(tilesetImage, -size / 2, -size / 2, size, size, 32 * ring_col, 32 * ring_row, 32, 32);
...
function drawMeteor(meteorIndex, scale) {
  ...
  // Draw the base planet
  image(tilesetImage, -size / 2, -size / 2, size, size, 32 * meteor_col, 32 * meteor_row, 32, 32);

    
    
  pop(); // Restore previous drawing sMeteor
}
...
function drawBread(eaten) {
  ...
  image(tilesetImage, -16, -16, 32, 32, 32 * col, 32 * row, 32, 32);
